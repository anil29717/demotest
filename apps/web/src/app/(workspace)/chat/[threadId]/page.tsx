...
export default function ChatThreadPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([sum, msgRes]) => {
        ...
        setMessages((msgRes?.messages ?? []).slice().sort(
  ...
  const refreshPresence = useCallback(async () => {
    if (!token || !summary?.participants?.length) return;
    ...
    setPresence(next);
  }, [token, summary?.participants, user?.id]);

  useEffect(() => {
    void refreshPresence();
    const t = setInterval(() => void refreshPresence(), 45000);
  ...
  return (
    <div className="mx-auto flex min-h-[min(720px,calc(100vh-6rem))] max-w-3xl flex-col rounded-xl border border-zinc-800 bg-zinc-950/40">
      ...
      >
        {messages.map((m) => {
          ...
          const senderRole =
            summary?.participants.find((p) => p.id === m.senderId)?.role ?? "";
